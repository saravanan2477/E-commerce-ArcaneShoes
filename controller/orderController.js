...
module.exports = {
  ...
  orderDetailsget: async (req, res) => {
    ...
    try {
      ...
      const originalPrice = await Product.findById(productid).select("price");
      ...
      if (productOfferInstance) {
        ...
        discountAmount = (parseFloat(originalPrice.price) * discountPercentage) / 100;
      }
  
      // Retrieve category offer
      const categoryOfferInstance = await categoryOffer.findOne({ category: { $regex: new RegExp(Category.category, 'i') }});    
  
      // Calculate category discount
      if (categoryOfferInstance) {
        const discountPercentage = parseFloat(categoryOfferInstance.alloffer);
        const categoryDiscountAmount = (parseFloat(originalPrice.price) * discountPercentage) / 100;
  ...
  cancelOrder: async (req, res) => {
    try {
      ...
      if (order.paymentMethod === "Net Banking") {
        ...
        // If a coupon was applied, refund the discounted amount
        if (order.Discount > 0) {
